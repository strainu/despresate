...
def find_siruta():
	...
	index = _csv._data[elem]["name"].translate(dia_trans).replace("MUNICIPIUL ", "").replace("ORAS ", "").replace("ORASUL ", "").replace("-", " ") + \
		_csv.get_county_string(elem).translate(dia_trans).replace("-", " ")
        if _csv._data[elem]['type'] <= 5:  # communes
	...
	if line[0][:7] == "JUDETUL" or line[0] == "MUNICIPIUL BUCURESTI":
	    ...
	    index = line[0].replace("MUNICIPIUL ", "").replace("ORAS ", "").replace("ORASUL ", "").replace("-", " ") + county
	if index in data:
