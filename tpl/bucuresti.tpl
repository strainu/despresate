...
<table cellpadding="0" cellspacing="0">
  <td valign="top" style="min-width:100px">
    <div class="leftbar" itemscope itemtype="http://schema.org/AdministrativeArea">
      ...
      <div class="leftbartitle" itemprop="name">Consiliul general</div>
        <table cellspacing="0" cellpadding="0" width="100%" id="cj" class="leftbarelem">
         <tr><th>Primar general</th><td itemprop="employee">{if $cjpresid}<a href="http://agenda.grep.ro/person/{$cjpresid}" title="Date de contact pentru {$cjpres}">{$cjpres}</a>{else}{$cjpres}{/if}</td></tr>
  ...
  <td valign=top>
	<div class="mainsection">
		...
		<p><b>{$name}</b> este capitala României. Se află în regiunea administrativă {$region} și 
		are statut de județ. În coloana din stânga aveți la dispoziție câteva informații de bază 
		despre județ, iar mai jos sunt prezente detalii despre economia, demografia și cultura 
		Bucureștiului.</p>
    </div>
    <div class="mainsection">
		<a name="adm" />
		...
		<p>
		{if $prpres || $prvice}
			{if $prpres}Prefectul Municipiului {$shortname} este <i>{$prpres}</i>, numit de guvern în anul {$pryear}. {/if}
			{if $prvice}{if $pryear == $prviceyear}Tot din{else}Din{/if} {$prviceyear}, viceprefect este <i>{$prvice}</i>. {/if}
		{else}
		Nu dispunem deocamdată de informatii despre prefectul și subprefectul Municipiului {$shortname}. Dacă aveți aceste date, vă rugăm să ne contactati.
		{/if}
		</p>
		<p>Consiliul general rezultat în urma alegerilor din {$cjpresyear} are următoarea componență:</p>
		<ul class="cjcouncil">
			<li><b>Primar (echivalentul președintelui CJ):</b> {if $cjpresid}<a href="http://agenda.grep.ro/person/{$cjpresid}" title="Date de contact pentru {$cjpres}">{$cjpres}</a>{else}{$cjpres}{/if} {if $cjpresparty}({$cjpresparty}){/if}</li><!--TODO: partid-->
			<li><b>Vicepreședinți:</b>
				...
				{else} Nu dispunem deocamdată de numele vicepreședinților consiliului general {$shortname}. Dacă dețineți aceste date, vă rugăm să ne contactați.
				{/if}
			</li>
			<li><b>Consilieri:</b> {if $cjcouncil}<ul>{foreach $cjcouncil as $member}<li>{if $member.id}<a href="http://agenda.grep.ro/person/{$member.id}" title="Date de contact pentru {$member.name}">{$member.name}</a>{else}{$member.name}{/if} {if $member.party}({$member.party}){/if}</li>{/foreach}</ul>
						{else} Nu dispunem deocamdată de componența consiliului general {$shortname}. Dacă dețineți aceste date, vă rugăm să ne contactați.
						{/if}</li>
		</ul>
    </div>
    <div class="mainsection">
		<a name="uat" />
		<div class="maintitle">Sectoare<a href="#top" class="toplink small">[sus]</a></div>
		<p>{$name} este împărțit în 6 sectoare conduse de un primar și un consiliu local:</p>
    ...
    <div class="mainsection">
		...
		<p>Nu avem încă informații legate de economia municipiului {$shortname}.</p>
    </div>
    <div class="mainsection">
		<a name="edu" />
